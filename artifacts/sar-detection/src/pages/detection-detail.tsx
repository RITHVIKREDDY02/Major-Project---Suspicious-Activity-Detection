    ...
    <div className="max-w-5xl mx-auto space-y-6 pb-20">
      ...
      <Card className="overflow-hidden border-border bg-black/40">
        <div className="relative aspect-video w-full flex items-center justify-center border-b border-border shadow-inner group">
          ...
          {/* Bounding Box overlay logic based on saved data or simulated */}
          {isSuspicious && (
            <div className="absolute border-2 border-dashed border-destructive bg-destructive/10 transition-all shadow-[0_0_15px_rgba(239,68,68,0.5)]"
                 style={{ top: '25%', left: '35%', width: '30%', height: '50%' }}>
              <div className="absolute top-0 left-0 -translate-y-full bg-destructive text-destructive-foreground px-2 py-1 text-xs font-mono font-bold whitespace-nowrap z-10">
                {detection.activityType?.toUpperCase()} {(detection.confidence! * 100).toFixed(1)}%
              </div>
            </div>
          )}
          {!isSuspicious && (
            <div className="absolute border-2 border-dashed border-green-500 bg-green-500/10 transition-all shadow-[0_0_15px_rgba(34,197,94,0.5)]"
                 style={{ top: '30%', left: '40%', width: '20%', height: '60%' }}>
              <div className="absolute top-0 left-0 -translate-y-full bg-green-500 text-white px-2 py-1 text-xs font-mono font-bold whitespace-nowrap z-10">
