...
async function getTwilioConfig(userId: number | null) {
  if (userId) {
...
async function getTelegramConfig(userId: number | null) {
  if (userId) {
    const [user] = await db.select().from(usersTable).where(eq(usersTable.id, userId));
...
router.post("/alerts/call", async (req, res) => {
  ...
  const [twilioConfig, telegramConfig] = await Promise.all([
    getTwilioConfig(userId),
